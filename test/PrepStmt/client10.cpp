...
void * mysql_thread(const std::string& username, int tid) {
	...
	unsigned int bl=0;
	for(;;) {
		if (!mysql_real_connect(mysql,"127.0.0.1",username.c_str(),"",SCHEMA,6033,NULL,0))
		...
		/* Here is a random delay to put some of the working threads into the idle state
		 *
		 * update global_variables set variable_value=4000 where variable_name='mysql-wait_timeout';
		 * update global_variables set variable_value=1000 where variable_name='mysql-session_idle_ms';
		 * update global_variables set variable_value=1 where variable_name='mysql-max_stmts_per_connection';
		 */
